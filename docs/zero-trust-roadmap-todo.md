...
- Completed: Hardened runtime invariant enforcement by making the non-Verus `RuntimeBigNatWitness` field `limbs_le` private in `src/runtime_bigint_witness/mod.rs`, so external runtime code cannot construct non-canonical witness values via struct literals.
- Completed: Added a `scripts/check.sh` guard (`check_runtime_big_nat_field_privacy`) that fails if the non-Verus `RuntimeBigNatWitness` declaration reintroduces a public `limbs_le` field.
- Failed attempt (adjusted): making the Verus-path `RuntimeBigNatWitness` fields private as well caused Verus to reject public open specs with `field expression for an opaque datatype` in `src/runtime_bigint_witness/verified_impl.rs`; kept Verus fields public while retaining non-Verus field privacy.
- Completed verification attempt: `./scripts/check.sh --require-verus --forbid-rug-normal-deps --forbid-trusted-escapes --target-a-strict-smoke --min-verified 89` passes after the runtime-field privacy hardening (runtime tests 4/4; baseline and strict-feature Verus both report `89 verified, 0 errors`; verified-count gate passes).
- Completed verification attempt: `cargo test --manifest-path Cargo.toml --features rug-oracle` passes after the runtime-field privacy hardening (6/6 tests).
- Completed: Added a CI/docs drift preflight in `scripts/check.sh` (`check_ci_strict_gate_alignment`) that validates the CI-equivalent strict command in `.github/workflows/check.yml` and `README.md` stays aligned, including `--min-verified`.
- Completed: Added an explicit CI-equivalent strict-check command in `README.md` so local docs and workflow use the same gate invocation.
- Failed attempt (fixed): initial `--min-verified` extraction in the new preflight used `rg` replacement with options ordered incorrectly after `--`, causing false parse failures; fixed by extracting the numeric token in two regex steps.
- Completed verification attempt: `./scripts/check.sh --require-verus --forbid-rug-normal-deps --forbid-trusted-escapes --target-a-strict-smoke --min-verified 89` passes after CI/docs alignment hardening (runtime tests 4/4; baseline and strict-feature Verus both report `89 verified, 0 errors`; verified-count gate passes).
- Completed: Added a CI toolchain-drift preflight in `scripts/check.sh` (`check_ci_toolchain_alignment`) that parses `.github/workflows/check.yml` and fails if `rustup toolchain install`/`rustup default` drift from each other or from `scripts/check.sh` `TOOLCHAIN`.
- Completed: Updated `README.md` and `docs/runtime-bigint-trust-assumptions.md` to document that local strict checks now also enforce workflow toolchain-pin alignment.
...
- Completed: Eliminated `#[verifier::exec_allows_no_decreases_clause]` from `src/runtime_bigint_witness/verified_impl.rs` by adding explicit loop termination measures (`decreases`) for all affected exec loops.
- Completed: Hardened `scripts/check.sh --forbid-trusted-escapes` to reject `#[verifier::exec_allows_no_decreases_clause]` in non-test `src/` files, extending the trusted-escape source-tree gate.
- Completed: Updated `README.md` and `docs/runtime-bigint-trust-assumptions.md` to document the strengthened trusted-escape policy and no-decreases-escape elimination.
- Failed attempt (fixed): first pass placed `decreases` clauses before `invariant` in Verus `while` blocks, which produced a parser error (`expected curly braces` at `src/runtime_bigint_witness/verified_impl.rs:1324`); fixed by ordering each loop header as `invariant ...` followed by `decreases ...`.
- Completed verification attempt: `./scripts/check.sh --require-verus --forbid-rug-normal-deps --forbid-trusted-escapes --target-a-strict-smoke --min-verified 89` passes after removing no-decreases escapes and tightening the trusted-escape gate (runtime tests 4/4; baseline + strict-feature Verus each report `89 verified, 0 errors`; verified-count floor/parity gates pass).
- Completed verification attempt: `cargo test --manifest-path Cargo.toml --features rug-oracle` passes after removing no-decreases escapes and tightening the trusted-escape gate (6/6 tests).
- Completed: Added `--rug-oracle-tests` to `scripts/check.sh`, enabling strict pipelines to run differential/oracle coverage (`cargo test --features rug-oracle`) in addition to baseline runtime tests.
- Completed: Hardened CI/docs strict-command drift gating so `check_ci_strict_gate_alignment` now requires `--rug-oracle-tests` in the CI-equivalent strict command.
- Completed: Updated `.github/workflows/check.yml`, `README.md`, and `docs/runtime-bigint-trust-assumptions.md` so strict checks explicitly include and document `--rug-oracle-tests`.
- Completed verification attempt: `./scripts/check.sh --require-verus --forbid-rug-normal-deps --forbid-trusted-escapes --rug-oracle-tests --target-a-strict-smoke --min-verified 89` passes after oracle-gate wiring (runtime tests 4/4; rug-oracle tests 6/6; baseline + strict-feature Verus each report `89 verified, 0 errors`; verified-count floor/parity gates pass).
- Completed: Added CI checkout-wiring preflight in `scripts/check.sh` (`check_ci_workflow_checkout_wiring`) that fails if `.github/workflows/check.yml` drifts from required repository setup (`Checkout verus-bigint` path, `Checkout Verus` repository/path, and checkout ordering before build/strict steps).
...
- Completed: Hardened `scripts/check.sh` CI verify-job/step execution preflights so strict checks now also fail if the `verify` job sets job-level `continue-on-error`, or if critical steps (`Build Verus tools`, `Run strict checks`) add step-level `if:` gating.
- Completed: Updated `README.md` and `docs/runtime-bigint-trust-assumptions.md` to document the strengthened CI execution-contract/fail-fast preflights (job-level `continue-on-error` rejection + step-level `if:` rejection).
- Completed verification attempt: `./scripts/check.sh --require-verus --forbid-rug-normal-deps --forbid-trusted-escapes --rug-oracle-tests --target-a-strict-smoke --min-verified 89` passes after execution-contract hardening (runtime tests 4/4; rug-oracle tests 6/6; baseline + strict-feature Verus each report `89 verified, 0 errors`; verified-count floor/parity gates pass).
- Failed/blocked attempt: direct GitHub-hosted CI execution remains unobservable in this sandbox (`gh`/`act` unavailable in PATH), so the `in CI` exit criterion is still not directly witnessed despite the added preflight hardening.
- Completed: Added CI toolchain-install preflight in `scripts/check.sh` (`check_ci_toolchain_install_wiring`) so strict checks now fail if `.github/workflows/check.yml` drifts from required `Install required Rust toolchain` wiring (step exists, stays ordered before build/strict checks, stays fail-fast, installs pinned toolchain with `--profile minimal` and required components, and sets matching `rustup default`).
- Completed: Updated `README.md` and `docs/runtime-bigint-trust-assumptions.md` to document the new CI toolchain-install wiring preflight guarantees.
- Completed verification attempt: `./scripts/check.sh --require-verus --forbid-rug-normal-deps --forbid-trusted-escapes --rug-oracle-tests --target-a-strict-smoke --min-verified 89` passes after toolchain-install preflight hardening (runtime tests 4/4; rug-oracle tests 6/6; baseline + strict-feature Verus each report `89 verified, 0 errors`; verified-count floor/parity gates pass).
- Completed: Hardened `scripts/check.sh --forbid-trusted-escapes` so non-test `src/` files are now also rejected if they contain `unsafe`, extending the strict trusted-surface gate beyond Verus-specific escape hatches.
...
- Completed: Switched to a true single-implementation model by removing `src/runtime_bigint_witness/runtime_impl.rs` and compiling only `src/runtime_bigint_witness/verified_impl.rs` under `cfg(verus_keep_ghost)`.
