...
<file>              path relative to the repo root (e.g. src/runtime_bigint_witness/verified_impl/ops_add.rs)
[function_pattern]  optionally restrict verification to matching functions

examples:
  ./scripts/check-single-file.sh src/runtime_bigint_witness/verified_impl/ops_add.rs
  ./scripts/check-single-file.sh src/runtime_bigint_witness/verified_impl/ops_mul.rs
  ./scripts/check-single-file.sh src/runtime_bigint_witness/verified_impl/ops_div_rem.rs lemma_div
  ./scripts/check-single-file.sh src/runtime_bigint_witness/signed_verified_impl.rs
EOF
  exit 0
fi
...
#   src/runtime_bigint_witness/verified_impl/ops_add.rs  ->  runtime_bigint_witness::verified_impl::ops_add
#   src/runtime_bigint_witness/mod.rs                    ->  runtime_bigint_witness
...
run_cargo_verus() {
  if command -v rustup >/dev/null 2>&1; then
    ...
    cd "$ROOT_DIR"
    cargo verus verify --manifest-path Cargo.toml -p verus-bigint \
